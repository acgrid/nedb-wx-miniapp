 ...
 */
const existsAsync = path => new Promise((resolve) => fsm.access({ path, success: () => resolve(true), fail: () => resolve(false) }))
...
const crashSafeWriteFileLinesAsync = async (filename, lines, modes = { fileMode: DEFAULT_FILE_MODE, dirMode: DEFAULT_DIR_MODE }) => {
  ...
  await flushToStorageAsync({ filename: path.dirname(filename), ensureDatafileIntegrityAsyncisDir: true, mode: modes.dirMode })
}
