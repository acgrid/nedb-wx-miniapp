...
This package is a WeChat Mini Program port of that fork. It keeps the NeDB API
surface but publishes a single CommonJS bundle that uses
`wx.getFileSystemManager()` for persistence and has no runtime npm
dependencies.

**Embedded persistent or in memory database for WeChat Mini Programs, 100%
JavaScript, no binary dependency**. API is a subset of MongoDB's and it's
[plenty fast](#speed).

## Installation

Module name on npm is [`@acgrid/nedb-wx-miniapp`](https://www.npmjs.com/package/@acgrid/nedb-wx-miniapp).

```
npm install @acgrid/nedb-wx-miniapp
```

Then to import, you just have to:

```js
const Datastore = require('@acgrid/nedb-wx-miniapp')
```
...
const doc = {
  ...
  infos: { name: '@acgrid/nedb-wx-miniapp' }
}
...
try {
  ...
  await db.insertAsync({ somefield: '@acgrid/nedb-wx-miniapp' })
  // works
  await db.insertAsync({ somefield: '@acgrid/nedb-wx-miniapp' })
  //rejects
} catch (error) {
...
This npm package is built for WeChat Mini Programs. The published package exposes
`dist/nedb.wx.js` as its only runtime entry and does not publish the Node.js,
browser, or React Native source variants.

The Mini Program bundle:

* stores persistent data with `wx.getFileSystemManager()`;
* maps relative filenames into `wx.env.USER_DATA_PATH`;
* replaces Node-only modules such as `fs`, `crypto`, `stream`, `events`, and
  `util` with Mini Program-safe implementations bundled into the single output
  file;
* has no runtime npm dependencies.

After installing the package inside your Mini Program root, run the WeChat
Developer Tools npm build step and import it normally:

```js
const Datastore = require('@acgrid/nedb-wx-miniapp')
```

The package is not intended to be used as the Node.js, Electron, browser, or
React Native version of NeDB. Use the upstream packages for those environments.
...
This Mini Program port of NeDB will be incrementally updated to:
* [ ] keep the WeChat Mini Program bundle compatible with DevTools npm build;
* [ ] document data-file diagnostics and backup guidance for Mini Program apps;
* [ ] remove deprecated upstream features that are not useful in Mini Program use;
